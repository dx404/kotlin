...
public:
    explicit HeapGrowthController(GCSchedulerConfig& config) noexcept : config_(config), targetHeapBytes_(config.targetHeapBytes.load(std::memory_order_relaxed)) {}

    // Called by the mutators.
    // Returns true if needs GC.
    bool OnAllocated(size_t totalAllocatedBytes) noexcept {
        return totalAllocatedBytes >= targetHeapBytes_;
    }
